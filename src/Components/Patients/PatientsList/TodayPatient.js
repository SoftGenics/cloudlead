...
function TodayPatient() {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () =>{
      const res = await axios.get("https://backend.softgenics.in/api/todaypatient");
      SetPost(res.data)
    }
    fetchPosts();
  });
  ...
  return (
    ...
    <Container className='mt-5'>
      <h1 className='text-center mb-3'>Today <span style={{color:'#3bc0fd'}}> Patients</span></h1>
      ...
      return(
        <>
          <tr>
            ...
            <td>
              <Link to={`/doctor-profile/${getData.id}`} className='btn btn-primary  mx-3'>View &nbsp;&nbsp;<i className="far fa-eye"></i></Link>
